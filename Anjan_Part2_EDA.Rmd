...
Let's look at summary to quickly look at the statistcal percentiles for each variable
```{r}
summary(titanic)
...
```
Age looks skewed right with the biggest block of travllers being in their 20s a large amount of travelers also in their 30s. There weren't as many travellers with ages greater than 40 and less than 18. 
...
Let's look at mosaic plots of look at the correlation betwen Pclass and Sex with Survived as these appeared to be the most significant predictor variables from the above analysis.
```{r}
